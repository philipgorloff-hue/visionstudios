...
if(!isBFECC){ vec2 vel = texture2D(velocity,uv).xy; vec2 uv2 = uv - vel*dt*ratio; gl_FragColor = vec4(texture2D(velocity,uv2).xy,0.,0.); }
else { vec2 vel_old = texture2D(velocity,uv).xy; vec2 spot_old = uv - vel_old*dt*ratio; vec2 vel_new1 = texture2D(velocity,spot_old).xy; vec2 spot_new2 = spot_old + vel_new1*dt*ratio; vec2 err = spot_new2 - uv; vec2 spot_new3 = uv - err/2.0; vec2 vel_2 = texture2D(velocity,spot_new3).xy; gl_FragColor = vec4(texture2D(velocity,spot_new3-vel_2*dt*ratio).xy,0.,0.); } }`;
    const color_frag = `
precision highp float; uniform sampler2D velocity; uniform sampler2D palette; uniform vec4 bgColor; varying vec2 uv;
void main(){ vec2 vel = texture2D(velocity,uv).xy; float lenv = clamp(length(vel),0.,1.); vec3 c = texture2D(palette,vec2(lenv,0.5)).rgb; gl_FragColor = vec4(mix(bgColor.rgb,c,lenv), mix(bgColor.a,1.,lenv)); }`;
    const divergence_frag = `
precision highp float; uniform sampler2D velocity; uniform float dt; uniform vec2 px; varying vec2 uv;
void main(){ float x0=texture2D(velocity,uv-vec2(px.x,0.)).x; float x1=texture2D(velocity,uv+vec2(px.x,0.)).x; float y0=texture2D(velocity,uv-vec2(0.,px.y)).y; float y1=texture2D(velocity,uv+vec2(0.,px.y)).y; gl_FragColor=vec4((x1-x0+y1-y0)/2./dt); }`;
    const externalForce_frag = `
precision highp float; uniform vec2 force; uniform vec2 center; uniform vec2 scale; uniform vec2 px; varying vec2 vUv;
void main(){ vec2 circle=(vUv-0.5)*2.; float d=1.-min(length(circle),1.); d*=d; gl_FragColor=vec4(force*d,0.,1.); }`;
    const poisson_frag = `
precision highp float; uniform sampler2D pressure; uniform sampler2D divergence; uniform vec2 px; varying vec2 uv;
void main(){ float p0=texture2D(pressure,uv+vec2(px.x*2.,0.)).r; float p1=texture2D(pressure,uv-vec2(px.x*2.,0.)).r; float p2=texture2D(pressure,uv+vec2(0.,px.y*2.)).r; float p3=texture2D(pressure,uv-vec2(0.,px.y*2.)).r; float div=texture2D(divergence,uv).r; gl_FragColor=vec4((p0+p1+p2+p3)/4.-div); }`;
    const pressure_frag = `
...
void main(){ float p0=texture2D(pressure,uv+vec2(px.x,0.)).r; float p1=texture2D(pressure,uv-vec2(px.x,0.)).r; float p2=texture2D(pressure,uv+vec2(0.,px.y)).r; float p3=texture2D(pressure,uv-vec2(0.,px.y)).r; vec2 v=texture2D(velocity,uv).xy; gl_FragColor=vec4(v-vec2(p0-p1,p2-p3)*0.5*dt,0.,1.); }`;
    const viscous_frag = `
...
void main(){ vec2 old=texture2D(velocity,uv).xy; vec2 n0=texture2D(velocity_new,uv+vec2(px.x*2.,0.)).xy; vec2 n1=texture2D(velocity_new,uv-vec2(px.x*2.,0.)).xy; vec2 n2=texture2D(velocity_new,uv+vec2(0.,px.y*2.)).xy; vec2 n3=texture2D(velocity_new,uv-vec2(0.,px.y*2.)).xy; gl_FragColor=vec4((4.*old+v*dt*(n0+n1+n2+n3))/(4.*(1.+v*dt)),0.,0.); }`;
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(props) {
        ...
        const sx = props.cursor_size * props.cellScale.x, sy = props.cursor_size * props.cellScale.y;
    ...
    class Viscous extends ShaderPass {
      ...
      update({ viscous: v, iterations, dt }) {
        this.uniforms.v.value = v;
        let fin, fout;
        for (let i = 0; i < iterations; i++) {
          fin = i%2===0 ? this.props.output0 : this.props.output1;
          fout = i%2===0 ? this.props.output1 : this.props.output0;
          this.uniforms.velocity_new.value = fin.texture;
          this.props.output = fout; this.uniforms.dt.value = dt; super.update();
        }
        return fout;
      }
    }
    ...
    class Divergence extends ShaderPass {
      constructor(p) {
        super({ material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: { boundarySpace: { value: p.boundarySpace }, velocity: { value: p.src.texture }, px: { value: p.cellScale }, dt: { value: p.dt } } }, output: p.dst });
        this.init();
      }
    ...
    class Poisson extends ShaderPass {
      constructor(p) {
        super({ material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: p.boundarySpace }, pressure: { value: p.dst_.texture }, divergence: { value: p.src.texture }, px: { value: p.cellScale } } }, output: p.dst, output0: p.dst_, output1: p.dst });
        this.init();
      }
      update({ iterations }) {
        let pin, pout;
        for (let i = 0; i < iterations; i++) {
          ...
          this.uniforms.pressure.value = pin.texture; this.props.output = pout; super.update();
        }
        return pout;
      }
    }
    ...
    class Pressure extends ShaderPass {
      constructor(p) {
        super({ material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: { boundarySpace: { value: p.boundarySpace }, pressure: { value: p.src_p.texture }, velocity: { value: p.src_v.texture }, px: { value: p.cellScale }, dt: { value: p.dt } } }, output: p.dst });
        this.init();
      }
  ...
  return (
    <div
      ref={mountRef}
      className={className ? `liquid-ether-container ${className}` : undefined}
      style={{
        position: 'relative',
        overflow: 'hidden',
        width: '100%',
